...
public class CloseSessionProxy {

    @RuntimeType
    public static void onSessionClose(@This final RemoteWebDriver driver, @SuperCall final Callable<Boolean> proxy) throws Exception {
        ...
        for (WebDriverListener listener : AgentListenerHolder.getWebDriverListeners()) {
            listener.onSessionClose(context);
        }
    }
}
